...
namespace AxisBank.lib.Core
{
    public static class AccountRepository
    {
        ...
        public static bool depositToAccount(string accnum , decimal amount)
        {
            // getting account all accounts
            string path = file.accountPath;
            List<string> data = new List<string>();
            data = FileRepository.readFile(path);
            ...
            foreach (var item in data)
            {
                if (item.Split(",")[4] == accnum)
                {
                    string balance = item.Split(",")[7];
                    decimal dbalance = decimal.Parse(balance);
                    dbalance += amount;
                    dbalance.ToString();
                }
            }
