...
namespace AxisBank.lib.Core
{
    public static class AccountRepository
    {
        ...
        /// </summary>
        public static string checkBalance(string custId , string accType)
        {
            string balance = "";
            string accountnumber = "";
            List<string> costAcc = new List<string>();
            List<string> specificAccnt = new List<string>();
            costAcc = getAccountById(custId);

            if (accType == "Saving Account")
            {
                accountnumber = costAcc[0].Split(",")[4];
                specificAccnt.Add( costAcc[0]);
            }
            else
            {
                accountnumber = costAcc[1].Split(",")[4];
                specificAccnt.Add(costAcc[1]);
            }

            foreach (var item in specificAccnt)
            {
